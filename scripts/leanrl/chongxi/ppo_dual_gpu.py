...
def main(args):
    ...
    for step in range(args.num_steps):
        ...
        action, logprob, _, value, mu, sigma = inference_agent.get_action_and_value(
            next_obs_nn
        )
        ...
        rewards[step].copy_(
            reward.to(nn_device, non_blocking=True).detach().view(-1, 1)
        )
        dones[step].copy_(
            next_done.to(nn_device, non_blocking=True).detach().view(-1, 1)
        )

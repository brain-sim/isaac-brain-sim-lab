...
def main(args):
    ...
    if args.agent_type == "CNNPPOAgent":
        ...
        print(
            colored(
                f"CNNPPOAgent, n_obs:{n_obs} n_act:{n_act}, img_size:{args.img_size}",
                "green",
                attrs=["bold"],
            )
        )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if global_step_burnin is not None and iteration % args.checkpoint_interval == 0:
            ...
            # Also save the latest checkpoint directly in chongxi folder
            latest_ckpt_path = os.path.join(
                os.path.dirname(os.path.abspath(__file__)), "latest_checkpoint.pt"
            )
            torch.save(checkpoint_data, latest_ckpt_path)
            print(
                colored(
                    f"Saved latest checkpoint to {latest_ckpt_path}",
                    "cyan",
                    attrs=["bold"],
                )
            )
            ...
            if mean_return > best_return:
                ...
                # Also save best checkpoint in chongxi folder
                best_ckpt_path = os.path.join(
                    os.path.dirname(os.path.abspath(__file__)), "best_checkpoint.pt"
                )
                torch.save(checkpoint_data, best_ckpt_path)
                print(
                    colored(
                        f"Saved best checkpoint (return={best_return:.2f}) to {best_ckpt_path}",
                        "green",
                        attrs=["bold"],
                    )
                )
...
# python scripts/torchrl/ppo_continuous_action.py --headless --task Spot-Nav-Avoid-v0 --num_envs 128 --total_timesteps 10_000_000 --log_interval 1 --checkpoint_interval 1_000  --log --ent_coef 0.000001

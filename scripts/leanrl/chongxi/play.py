...
class ExperimentArgs:
    ...
    checkpoint_path: str = ""
    """path to a specific checkpoint to load (overrides checkpoint_type if provided)"""
...
def main(args):
    ...
    if args.checkpoint_path:
        ...
        print(
            colored(
                f"[INFO] : Loading specific checkpoint from {checkpoint_path}",
                "green",
                attrs=["bold"],
            )
        )
    else:
        # Use latest or best checkpoint from chongxi folder
        script_dir = os.path.dirname(os.path.abspath(__file__))
        if args.checkpoint_type == "best":
            checkpoint_path = os.path.join(script_dir, "best_checkpoint.pt")
            print(
                colored(
                    f"[INFO] : Loading best checkpoint from {checkpoint_path}",
                    "green",
                    attrs=["bold"],
                )
            )
        else:  # default to latest
            checkpoint_path = os.path.join(script_dir, "latest_checkpoint.pt")
            print(
                colored(
                    f"[INFO] : Loading latest checkpoint from {checkpoint_path}",
                    "green",
                    attrs=["bold"],
                )
            )
    ...
    if "global_step" in checkpoint:
        print(
            colored(
                f"[INFO] : Checkpoint from global step: {checkpoint['global_step']}",
                "yellow",
                attrs=["bold"],
            )
        )
...
# python ./scripts/leanrl/chongxi/play.py --task Brain-Sim-Spot-Nav-Avoid-v0 --num_eval_envs 16 --enable_cameras

# Load best checkpoint:
# python ./scripts/leanrl/chongxi/play.py --task Brain-Sim-Spot-Nav-Avoid-v0 --num_eval_envs 16 --enable_cameras --checkpoint_type best

# Load specific checkpoint:
# python ./scripts/leanrl/chongxi/play.py --task Brain-Sim-Spot-Nav-Avoid-v0 --num_eval_envs 16 --enable_cameras --checkpoint_path ./path/to/checkpoint.pt

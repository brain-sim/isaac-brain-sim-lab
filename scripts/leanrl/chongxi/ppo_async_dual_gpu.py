...
class AsyncDualGPURollout:
    ...
    def nn_worker(self, storage, num_steps):
        ...
        for step in range(num_steps):
            # NN inference
            with torch.no_grad():
                action, logprob, _, value, mu, sigma = self.agent.get_action_and_value(
                    next_obs
                )
            ...
            if step < num_steps - 1:
                ...
                storage["rewards"][step].copy_(
                    reward.to(self.nn_device).detach().view(-1, 1)
                )
                storage["dones"][step].copy_(
                    done.to(self.nn_device).detach().view(-1, 1)
                )

    def rollout(self, storage, num_steps):
        """Execute parallel rollout with physics and NN on separate GPUs"""
        # Start physics thread
        physics_thread = threading.Thread(target=self.physics_worker, args=(num_steps,))
...
def main_async(args):
    ...
    storage = {
        ...
        "actions": torch.zeros((args.num_steps, args.num_envs, n_act)).to(
            args.nn_device
        ),
        ...
        "dones": torch.zeros((args.num_steps, args.num_envs, 1))
        .to(args.nn_device)
        .byte(),

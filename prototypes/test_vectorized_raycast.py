...
def test_vectorized_raycast():
    ...
    # Compare results
    print(f"\nComparison:")
    print(f"Max difference: {torch.max(torch.abs(bilinear_distances - raycast_distances)):.6f}")
    print(f"Mean difference: {torch.mean(torch.abs(bilinear_distances - raycast_distances)):.6f}")
    ...
    large_test_positions = torch.rand(1000, 2, device=device) * 4 + 0.5  # Random positions in maze

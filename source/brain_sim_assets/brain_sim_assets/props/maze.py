...
class bsMazeGenerator:
    ...
    def create_example_maze(
        ...
        if maze_config_path_in is None:
            maze_config_path = (
                f"{BRAIN_SIM_ASSETS_PROPS_CONFIG_DIR}/example_config.json"
            )
        else:
            maze_config_path = maze_config_path_in

        return cls(
            maze_txt_path, maze_config_path, position_offset, create_wall_on_init
        )
    ...
    def setup(self):
        if self._maze_config:
            ...
            if self._maze_txt_path:
                ...
                world_start_x = (
                    start_pos[1] * self._cell_size + self._position_offset[0]
                )
                world_start_y = (
                    start_pos[0] * self._cell_size + self._position_offset[1]
                )
            ...
            for j in range(width):
                if self.is_wall(maze_grid[i][j]):
                    ...
                    wall_cfg = bsBrickGenerator.get_brick_object(
                        ...
                        rot=(1, 0, 0, 0),
                        scale=(self._cell_size, self._cell_size, self._wall_height),
    ...
    def get_wall_collection(self) -> RigidObjectCollectionCfg:
        ...
        return RigidObjectCollectionCfg(rigid_objects=wall_dict)

...
class EnvComponentObservation:
    ...
    def post_env_init(self):
        ...
        self._position_error = torch.zeros((self.env.num_envs), device=self.env.device, dtype=torch.float32)
        self._position_error_vector = torch.zeros((self.env.num_envs, 2), device=self.env.device, dtype=torch.float32)
        self._previous_position_error = torch.zeros((self.env.num_envs), device=self.env.device, dtype=torch.float32)
        self.target_heading_error = torch.zeros((self.env.num_envs), device=self.env.device, dtype=torch.float32)

    def get_image_obs(self) -> torch.Tensor:
        """Get camera image observations."""
        image_obs = self.env.env_component_robot.camera.data.output["rgb"].float().permute(0, 3, 1, 2) / 255.0
    ...
    def get_observations(self) -> dict:
        ...
        self._position_error_vector = (
            current_target_positions - self.env.env_component_robot.robot.data.root_pos_w[:, :2]
        )
        ...
        target_heading_w = torch.atan2(
            self.env.env_component_waypoint._target_positions[
                self.env.env_component_robot.robot._ALL_INDICES, 
                self.env.env_component_waypoint._target_index, 1
            ] - self.env.env_component_robot.robot.data.root_link_pos_w[:, 1],
            self.env.env_component_waypoint._target_positions[
                self.env.env_component_robot.robot._ALL_INDICES, 
                self.env.env_component_waypoint._target_index, 0
    ...
    def reset(self, env_ids):
        ...
        self._position_error_vector = (
            current_target_positions[:, :2] - self.env.env_component_robot.robot.data.root_pos_w[:, :2]
        )
        self._position_error = torch.norm(self._position_error_vector, dim=-1)
        self._previous_position_error = self._position_error.clone()
        ...
        heading = self.env.env_component_robot.robot.data.heading_w[:]
        target_heading_w = torch.atan2(
            self.env.env_component_waypoint._target_positions[:, 0, 1] - self.env.env_component_robot.robot.data.root_pos_w[:, 1],
            self.env.env_component_waypoint._target_positions[:, 0, 0] - self.env.env_component_robot.robot.data.root_pos_w[:, 0],
        )

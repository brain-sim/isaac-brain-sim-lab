...
class EnvComponentTermination:
    ...
    def post_env_init(self):
        ...
        self._vehicle_flipped = torch.zeros(
        ...
        self.termination_on_avoid_goal_collision = (
            self.env.cfg.termination_on_avoid_goal_collision
        )
    ...
    def check_flipped(self) -> torch.Tensor:
        ...
        world_up_vector = math.quat_apply(
            robot_quat, local_up.repeat(self.env.num_envs, 1)
        )
        world_up = torch.tensor([0.0, 0.0, 1.0], device=self.env.device)
        up_dot_product = torch.sum(world_up_vector * world_up, dim=-1)
        up_angle = torch.abs(torch.acos(torch.clamp(up_dot_product, -1.0, 1.0)))
    ...
    def get_dones(self) -> tuple[torch.Tensor, torch.Tensor, dict]:
        ...
        termination_infos = {
            ...
            "Episode_Termination/task_completed": self.env.task_completed.float()
            .mean()
            .item(),
        }

...
def goal_reach(
    env: ManagerBasedRLEnv, robot_cfg: SceneEntityCfg, threshold: float = 1.4
) -> torch.Tensor:
    ...
    goal_markers = [
        name for name in env.scene.keys() if name.startswith("goal_marker_")
    ]
    if not goal_markers:
    ...
    distances = torch.norm(goal_positions - robot_pos.unsqueeze(0), dim=-1)
...
def obstacle_reach(
    env: ManagerBasedRLEnv, robot_cfg: SceneEntityCfg, threshold: float = 0.6
) -> torch.Tensor:
    ...
    obstacle_markers = [
        name for name in env.scene.keys() if name.startswith("obstacle_marker_")
    ]
    ...
    distances = torch.norm(obstacle_positions - robot_pos.unsqueeze(0), dim=-1)
...
class BaseTerminationsCfg:
    ...
    goal_reach = DoneTerm(
        func=goal_reach, params={"robot_cfg": SceneEntityCfg("robot"), "threshold": 1.4}
    )
    obstacle_reach = DoneTerm(
        func=obstacle_reach,

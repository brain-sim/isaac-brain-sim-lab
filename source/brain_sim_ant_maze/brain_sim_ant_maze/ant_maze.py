...
class bsAntMaze:
    ...
    def __init__(self, maze_config: Optional[bsAntMazeConfig] = None):
        ...
        self._button_door_mapping = (
            {}
        )  # Dict mapping button positions to door positions
        self._door_states = (
            {}
        )  # Dict tracking door states (True = closed/wall, False = open)
    ...
    def build_from_txt(
        self, file_path: str, buttons_config_path: Optional[str] = None
    ) -> None:
    ...
    def load_button_door_mapping(self, config_path: str) -> None:
        ...
        for mapping in config["button_mapping"]:
            if "button" not in mapping or "door" not in mapping:
                raise ValueError(
                    "Invalid button mapping: missing 'button' or 'door' key"
                )
    ...
    def create_maze(self, maze_array: List[List[Union[int, str]]]) -> None:
        ...
        if not isinstance(maze_array, list) or not all(
            isinstance(row, list) for row in maze_array
        ):
        ...
        if self._start_pos:
            ...
            if (
                not (0 <= row < len(maze) and 0 <= col < len(maze[0]))
                or maze[row][col] == 1
            ):

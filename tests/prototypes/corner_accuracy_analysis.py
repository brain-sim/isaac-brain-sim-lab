...
def analyze_corner_accuracy():
    ...
    print("=" * 50)
    print(f"{'Point':<12} {'Bilinear':<10} {'Bicubic':<10} {'RayCast':<10} {'Bi-Ray':<10} {'Bc-Ray':<10}")
    print("-" * 65)
    ...
    for i, (x, y) in enumerate(corner_points):
        ...
        print(f"({x:4.1f},{y:4.1f}) {bi_dist:<10.4f} {bc_dist:<10.4f} {rc_dist:<10.4f} {bi_error:<10.4f} {bc_error:<10.4f}")
    ...
    im2 = axes[0, 1].imshow(distance_fields['Bicubic'], cmap='viridis', origin='lower', 
                          extent=extent, alpha=0.8)
    cs2 = axes[0, 1].contour(X, Y, distance_fields['Bicubic'], levels=fine_contours, 
                           colors='white', linewidths=1, alpha=0.7)
    axes[0, 1].clabel(cs2, inline=True, fontsize=6, fmt='%.1f')
    
    for x, y in corner_points:
        axes[0, 1].plot(x, y, 'ro', markersize=8, markeredgecolor='white', markeredgewidth=2)
    ...
    im3 = axes[1, 0].imshow(distance_fields['Ray Cast'], cmap='viridis', origin='lower', 
                          extent=extent, alpha=0.8)
    cs3 = axes[1, 0].contour(X, Y, distance_fields['Ray Cast'], levels=fine_contours, 
                           colors='white', linewidths=1, alpha=0.7)
    axes[1, 0].clabel(cs3, inline=True, fontsize=6, fmt='%.1f')
    
    for x, y in corner_points:
        axes[1, 0].plot(x, y, 'ro', markersize=8, markeredgecolor='white', markeredgewidth=2)
    ...
    im4 = axes[1, 1].imshow(error, cmap='RdBu', origin='lower', extent=extent, 
                          alpha=0.8, vmin=-0.3, vmax=0.3)
    
    # Error contours
    error_levels = [-0.2, -0.1, -0.05, 0.0, 0.05, 0.1, 0.2]
    cs4 = axes[1, 1].contour(X, Y, error, levels=error_levels, 
                           colors='black', linewidths=1, alpha=0.8)
    axes[1, 1].clabel(cs4, inline=True, fontsize=6, fmt='%.2f')
    
    for x, y in corner_points:
        axes[1, 1].plot(x, y, 'ko', markersize=8, markeredgecolor='white', markeredgewidth=2)
...
def analyze_contour_smoothness():
    ...
    method_funcs = [calc.get_distance_bilinear, calc.get_distance_bicubic, calc.get_distance_raycast]
    ...
    for idx, (name, method) in enumerate(zip(methods, method_funcs)):
        ...
        axes[idx].imshow(maze[1:4, 1:4], cmap='binary', origin='lower', 
                       extent=[1.5, 3.5, 1.5, 3.5], alpha=0.3)
        
        # Dense contour lines to show smoothness
        contour_levels = np.arange(0.1, 2.0, 0.1)
        cs = axes[idx].contour(X, Y, distances, levels=contour_levels, 
                             colors='blue', linewidths=1, alpha=0.8)
        
        # Highlight key contours
        key_contours = [0.5, 1.0]
        cs_key = axes[idx].contour(X, Y, distances, levels=key_contours, 
                                 colors='red', linewidths=3, alpha=1.0)

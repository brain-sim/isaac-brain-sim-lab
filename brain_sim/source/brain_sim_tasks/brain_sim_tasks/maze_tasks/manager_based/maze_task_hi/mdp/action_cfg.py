...
class HierarchicalSpotActionTerm(ActionTerm):
    ...
    def __init__(self, cfg: "HierarchicalSpotActionCfg", env: ManagerBasedEnv):
        ...
        self._raw_actions = torch.zeros((self._num_envs, 3), device=self._device, dtype=torch.float32)
        self._processed_actions = torch.zeros((self._num_envs, 12), device=self._device, dtype=torch.float32)
    ...
    def process_actions(self, actions: torch.Tensor):
        ...
        _actions[:, 1:] = torch.clamp(_actions[:, 1:], min=-self.steering_max, max=self.steering_max)
        ...
        projected_gravity = self._robot.data.projected_gravity_b  # [num_envs, 3] - body frame
